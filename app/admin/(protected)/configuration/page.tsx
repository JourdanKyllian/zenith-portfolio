...
export default function ConfigurationPage() {
  ...
  const initialSocials = AVAILABLE_SOCIALS.reduce(
    (acc, net) => ({ ...acc, [net.id]: "" }),
    {},
  );
  ...
  useEffect(() => {
    const fetchSettings = async () => {
      ...
      if (dbData) {
        ...
        setActiveNetworks(
          Object.keys(fetchedSocials).filter((k) => fetchedSocials[k] !== ""),
        );
      }
      setIsLoading(false);
    };
  ...
  const handlePasswordUpdate = async (e: React.FormEvent) => {
    ...
    if (newPassword.length < 6) {
      setPasswordStatus("error");
      setPassMessage({ text: "Au moins 6 caractères requis.", type: "error" });
      setTimeout(() => setPasswordStatus("idle"), 3000);
      return;
    }
  ...
  if (isLoading)
    return (
      <div className="flex items-center justify-center min-h-[50vh] text-z-blue">
    ...
    <>
      <header className="mb-10 relative z-10">
        ...
        <p className="font-body text-sm text-z-muted mt-1">
          Gérez les identifiants de votre compte.
        </p>
      </header>
          ...
          <section className="bg-z-card border border-z-border rounded-xl p-6 shadow-2xl">
            ...
            >
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-z-bg border border-z-border rounded-lg py-3 pl-12 pr-4 text-sm text-z-text focus:border-z-blue focus:outline-none"
                  />
                </div>
              </div>
              <div className="space-y-2">
                <label className="text-[10px] uppercase font-bold tracking-widest text-z-muted ml-1">
                  Lien du CV (Drive PDF)
                </label>
                <div className="relative">
                  <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-z-muted">
                    <LinkIcon size={16} />
              ...
              <SubmitButton
                ...
                idleText="Enregistrer les infos"
              />
            </form>
          </section>
          ...
          <section className="bg-z-card border border-z-border rounded-xl p-6 shadow-2xl">
            ...
            >
              ...
              <SubmitButton
                ...
                fullWidth
                className="py-3 text-xs"
                idleText="Modifier le mot de passe"
              />
            </form>
          </section>
        </div>
        ...
        <section className="bg-z-card border border-z-border rounded-xl p-6 shadow-2xl">
          <div className="mb-6 flex items-center justify-between">
            <div>
              ...
              <p className="text-[10px] text-z-muted uppercase tracking-widest mt-1">
                Pied de page public
              </p>
            </div>
          </div>
          ...
          >
            <DynamicSocialLinks
              links={socials}
              setLinks={setSocials}
              activeLinks={activeNetworks}
              setActiveLinks={setActiveNetworks}
            />
            {socialMessage && (
            ...
            <div className="pt-4 border-t border-z-border">
              <SubmitButton
                ...
                fullWidth
                className="py-3 text-xs"
                idleText="Sauvegarder les réseaux"
              />
            </div>
          </form>
        </section>
