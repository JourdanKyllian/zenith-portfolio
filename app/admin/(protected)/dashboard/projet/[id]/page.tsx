...
export default function EditProjetPage() {
  ...
  const handleUpdateProjet = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      await purgeCache();
      setMessage({ text: "Projet mis à jour !", type: 'success' });
  ...
  const handleSaveDetails = async () => {
    ...
    try {
      ...
      await purgeCache();
      setDeletedSpIds([]); setHasUnsavedChanges(false); setEditingSpId(null);
      await fetchData(); 
    ...
    <>
      <div className="w-full flex flex-col lg:flex-row gap-6 h-auto lg:h-[calc(100vh-4rem)]">
        ...
        <div className="flex-1 flex flex-col min-w-0 bg-z-card/80 border border-z-border rounded-xl shadow-xl overflow-hidden relative z-10">
          ...
          {/* Header Fixe Col 1 */}
          <header className="shrink-0 p-6 border-b border-z-border flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-z-card/50 backdrop-blur-md">
            <div className="flex items-center gap-4">
              ...
              <div className="min-w-0">
                <h1 className="font-display font-bold text-2xl uppercase tracking-wider text-white truncate" title={titre}>{titre}</h1>
                <p className="font-body text-xs text-z-muted mt-0.5">Édition du projet</p>
              </div>
            </div>
            <button type="button" onClick={handleUpdateProjet} disabled={isSubmitting} className="shrink-0 btn-blue px-5 py-2.5 rounded-lg flex items-center justify-center gap-2 text-[10px] font-bold tracking-widest shadow-lg hover:scale-105 transition-all disabled:opacity-50">
              <Save size={14} /> {isSubmitting ? '...' : 'Enregistrer'}
            </button>
          </header>

          {/* Contenu Scrollable Col 1 */}
        ...
        <div className="flex-1 flex flex-col min-w-0 bg-z-card/80 border border-z-border rounded-xl shadow-xl overflow-hidden relative z-10">
          ...
          <header className="shrink-0 p-6 border-b border-z-border flex items-center justify-between bg-z-card/50 backdrop-blur-md">
            ...
            {hasUnsavedChanges && (
              <button 
                onClick={handleSaveDetails} disabled={isSavingDetails}
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
            ...
            {/* FORMULAIRE RÉACTIF DU DÉTAIL ACTIF */}
            {activeSp && (
              <div className="bg-z-bg border border-z-blue/30 rounded-lg p-5 space-y-5 animate-in fade-in slide-in-from-top-4 mt-6">
