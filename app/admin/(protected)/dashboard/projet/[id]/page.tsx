...
export default function EditProjetPage() {
  ...
  const handleUpdateProjet = async (e: React.FormEvent) => {
    ...
    // 2. Si c'est libre, on met à jour
    const updatedProjet = {
      titre,
      slug,
  ...
  const handleCreateSousProjet = async () => {
    ...
    const { data, error } = await supabase.from('sousprojet').insert([newSp]).select().single();
    if (!error && data) {
      setSousProjets([...sousProjets, data as SousProjet]);
      setSpTitre(''); setSpDescription(''); setSpYoutube(''); setSpDrive('');
      setSpOrdre(spOrdre + 1); setShowSpForm(false);
    }
  };

  const handleDeleteSousProjet = async (id: number) => {
    if (!confirm('Voulez-vous vraiment supprimer ce sous-projet ?')) return;
    const { error } = await supabase.from('sousprojet').delete().eq('id', id);
    if (!error) setSousProjets(sousProjets.filter(sp => sp.id !== id));
  };

  if (isLoading) return <div className="min-h-screen bg-z-bg flex items-center justify-center text-z-blue">Chargement de l'éditeur...</div>;
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 xl:grid-cols-3 gap-8">
        <div className="xl:col-span-2 space-y-6">
          <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
            ...
            <button onClick={handleUpdateProjet} disabled={isSubmitting} className="btn-blue px-6 py-3 rounded-lg flex items-center justify-center gap-2 text-xs font-bold tracking-widest shadow-lg hover:scale-105 transition-all disabled:opacity-50">
        ...
        {/* COLONNE DROITE SOUS-PROJETS */}
        <div className="xl:col-span-1 space-y-6">
          <div className="bg-z-card border border-z-border rounded-xl p-6 shadow-xl sticky top-6">
            <div className="flex items-center justify-between mb-6">
              <h2 className="font-sub text-xs uppercase tracking-[0.2em] text-white flex items-center gap-2"><Video size={16} className="text-z-blue" /> Détails (Sous-projets)</h2>
                    ...
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <div className="flex items-center gap-3 text-z-muted">
                          <span className="flex items-center gap-1 text-[10px] font-bold uppercase"><ListOrdered size={12}/> {sp.ordre}</span>
                          {sp.youtube_url && <span title="A une vidéo YouTube" className="flex items-center"><Video size={12} /></span>}
                          {sp.drive_url && <span title="A un lien Drive" className="flex items-center"><HardDrive size={12} /></span>}
                        </div>
                      </div>
                      <button onClick={() => handleDeleteSousProjet(sp.id)} className="text-z-muted hover:text-red-400 p-1 transition-colors"><Trash2 size={14} /></button>
                    </div>
                  </div>
                ))
              )}
            ...
            {!showSpForm ? (
              <button onClick={() => setShowSpForm(true)} className="w-full py-3 border border-dashed border-z-blue/50 text-z-blue rounded-lg text-xs font-bold uppercase tracking-widest hover:bg-z-blue/5 transition-colors flex items-center justify-center gap-2"><Plus size={16} /> Ajouter un détail</button>
            ) : (
              <div className="bg-z-bg border border-z-blue/30 rounded-lg p-4 space-y-4">
                ...
                <div className="space-y-2"><input type="text" placeholder="Titre (ex: Teaser)*" value={spTitre} onChange={e => setSpTitre(e.target.value)} className="w-full bg-z-card border border-z-border rounded p-2 text-xs" /></div>
                <div className="space-y-2"><textarea placeholder="Description optionnelle..." value={spDescription} onChange={e => setSpDescription(e.target.value)} className="w-full bg-z-card border border-z-border rounded p-2 text-xs resize-none" rows={2} /></div>
                <div className="space-y-2"><input type="url" placeholder="URL iframe YouTube" value={spYoutube} onChange={e => setSpYoutube(e.target.value)} className="w-full bg-z-card border border-z-border rounded p-2 text-xs" /></div>
                <div className="space-y-2"><input type="url" placeholder="URL Google Drive" value={spDrive} onChange={e => setSpDrive(e.target.value)} className="w-full bg-z-card border border-z-border rounded p-2 text-xs" /></div>
                <div className="space-y-2"><label className="text-[10px] uppercase text-z-muted">Ordre d'affichage</label><input type="number" min="1" value={spOrdre} onChange={e => setSpOrdre(parseInt(e.target.value))} className="w-full bg-z-card border border-z-border rounded p-2 text-xs" /></div>
                <div className="flex gap-2 pt-2">
                  <button onClick={handleCreateSousProjet} disabled={!spTitre} className="flex-1 btn-blue py-2 rounded text-xs font-bold disabled:opacity-50">Ajouter</button>
                  <button onClick={() => setShowSpForm(false)} className="flex-1 bg-z-card border border-z-border text-white py-2 rounded text-xs font-bold hover:bg-white/5">Annuler</button>
                </div>
              </div>
            )}
          </div>
