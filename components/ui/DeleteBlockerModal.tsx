...
export default function DeleteBlockerModal({
  isOpen, title, dependencyCount, dependencyName, explanation,
  onClose, onForceDelete, forceDeleteLabel, isForceDeleting
      ...
      <div className="bg-z-card border border-amber-500/30 rounded-xl shadow-[0_0_40px_rgba(245,158,11,0.15)] max-w-lg w-full overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="flex items-center justify-between p-5 border-b border-z-border bg-amber-500/5">
          <div className="flex items-center gap-3 text-amber-400">
            <ShieldAlert size={20} />
            <h3 className="font-sub text-xs uppercase tracking-widest font-bold">Suppression Bloquée</h3>
          </div>
          <button onClick={onClose} disabled={isForceDeleting} className="text-z-muted hover:text-white transition-colors disabled:opacity-50">
            <X size={20} />
          </button>
        </div>
          ...
          <div className="flex items-start gap-4 p-4 rounded-xl bg-z-bg border border-z-border">
            ...
            <div>
              <p className="text-sm font-body text-white mb-1">
                Impossible de supprimer <strong className="font-display tracking-wide">"{title}"</strong> de manière classique.
              </p>
              <p className="text-xs text-z-muted">
                Ce contenu est actuellement lié à <strong className="text-amber-400">{dependencyCount} {dependencyName}</strong>. 
                Les bases de données relationnelles interdisent la suppression d'un élément parent s'il possède encore des enfants.
              </p>
            </div>
          </div>

          <div className="space-y-3">
            <h4 className="font-sub text-[10px] font-bold uppercase tracking-widest text-z-blue">Que voulez-vous faire ?</h4>
