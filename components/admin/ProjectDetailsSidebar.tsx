      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
        ...
        <div className="space-y-3">
          {sousProjets.length === 0 ? (
            <p className="text-sm text-z-muted italic text-center py-4">
              Aucun détail lié.
            </p>
          ) : (
            sousProjets.map((sp) => (
              <div
                ...
                className={`bg-z-bg border rounded-lg p-3 group transition-all duration-200 cursor-pointer ${
                  editingSpId === sp.id
                    ? "border-z-blue bg-z-blue/5"
                    : "border-z-border hover:border-z-blue/50"
                } ${draggedId === sp.id ? "opacity-40 scale-95 border-dashed border-z-blue" : ""} ${
                  dragOverId === sp.id && draggedId !== sp.id
                    ? "border-z-blue bg-z-blue/10 translate-y-1"
                    : ""
                }`}
              >
                <div className="flex justify-between items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 text-z-muted">
                      ...
                      {sp.youtube_url && (
                        <span title="Vidéo YouTube">
                          <Video size={12} />
                        </span>
                      )}
                      {sp.drive_url && (
                        <span title="Lien Drive">
                          <HardDrive size={12} />
                        </span>
                      )}
                    </div>
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    <button
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        requestDeleteSp(sp.id, sp.titre);
                      }}
        ...
        {activeSp && (
          <div className="bg-z-bg border border-z-blue/30 rounded-lg p-4 sm:p-5 space-y-5 animate-in fade-in slide-in-from-top-4 mt-6">
            ...
            <div className="space-y-2">
              <input
                type="text"
                placeholder="Titre (ex: Teaser, Making-of)"
                value={activeSp.titre || ""}
                onChange={(e) => updateActiveSp("titre", e.target.value)}
                className="w-full bg-z-card border border-z-border rounded p-3 text-sm focus:border-z-blue focus:outline-none"
              />
            </div>
            <div className="space-y-2">
              <RichTextEditor
                value={activeSp.description || ""}
                onChange={(val) => updateActiveSp("description", val)}
            ...
            <div className="space-y-2">
              <input
                type="url"
                placeholder="URL iframe YouTube (optionnel)"
                value={activeSp.youtube_url || ""}
                onChange={(e) => updateActiveSp("youtube_url", e.target.value)}
                className="w-full bg-z-card border border-z-border rounded p-3 text-sm focus:border-z-blue focus:outline-none"
              />
            </div>
            <div className="space-y-2">
              <div className="flex items-center justify-between mb-1">
                ...
                <span className="text-[9px] text-z-blue/70 italic px-2 py-0.5 bg-z-blue/5 rounded border border-z-blue/10">
                  Drive direct
                </span>
              </div>
              <input
                ...
                value={activeSp.drive_url || ""}
                onChange={(e) => updateActiveSp("drive_url", e.target.value)}
                className="w-full bg-z-card border border-z-border rounded p-3 text-sm placeholder:text-z-muted/30 focus:border-z-blue focus:outline-none"
              />
            </div>
            ...
            <div className="pt-4 border-t border-z-border">
              <button
                type="button"
                onClick={() => setEditingSpId(null)}
                className="w-full bg-z-card border border-z-border text-white py-3 rounded-lg text-xs font-bold uppercase tracking-widest hover:bg-white/5 cursor-pointer transition-colors"
              >
                Fermer l'éditeur
              </button>
            </div>
